...
class SDE(abc.ABC):
  ...
  def discretize(self, diff_input, x, side_info, t):
    ...
    """
    dt = 1 / self.N
    drift, diffusion = self.sde(diff_input, x, side_info, t)
    f = drift * dt
  ...
  def reverse(self, score_fn, probability_flow=False):
    ...
    class RSDE(self.__class__):
      ...
      def sde(self, diff_input, x, side_info, t):
        ...
        score = score_fn(diff_input, x, side_info, t)
      ...
      def discretize(self, diff_input, x, side_info, t):
        ...
        f, G = discretize_fn(diff_input, x, side_info, t)
        rev_f = f - G[:, None, None] ** 2 * score_fn(diff_input, x, side_info, t) * (0.5 if self.probability_flow else 1.) #eq 42 second part
...
class VPSDE(SDE):
  def __init__(self, beta_min=0.0001, beta_max=0.5, N=1000):
...
class VESDE(SDE):
  def __init__(self, sigma_min=0.0001, sigma_max=0.5, N=1000):
    ...
    self.discrete_sigmas = torch.exp(torch.linspace(np.log(self.sigma_min), np.log(self.sigma_max), N)).to(device)
    self.N = N
  ...
  def discretize(self, diff_input, x, side_info, t):
    ...
    t = t.to(self.discrete_sigmas.device)  # Move t to the same device as self.discrete_sigmas
